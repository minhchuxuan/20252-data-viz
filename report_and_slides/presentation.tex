...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    {\small\color{slate}\itshape Full names and 8-digit student IDs
    \upshape $\cdot$ Supervisor: Tran Viet Trung}\\[10pt]
    {\color{gold}\rule{0.86\paperwidth}{2pt}}\\[14pt]
    {\footnotesize\color{slate} Dataset: Paris 2024 public CSVs (GitHub paris2024-data + data.gouv.fr, ODbL)
...
\begin{frame}{The dataset}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \small
      Eight public Paris~2024 CSVs, joined into analysis-ready tables and
      findable via Google Dataset Search:
      \begin{itemize}
        \item \teallead{11,183} athletes across \teallead{206} NOCs
        \item \teallead{45} disciplines, \teallead{2,271} athlete--medal records
      ...
      Quirks handled in code: semicolon + French + BOM enriched file,
      an unnamed index column, NOC$\rightarrow$ISO-3 mapping, French
      discipline/gender translation.
    \end{column}
...
\begin{frame}{Objective and story arc}
  ...
  demographics, and the spatial footprint of the Games.\\[4pt]
  \textbf{Goal.} A guided, interactive story that pairs \teallead{explanatory}
  narrative with \teallead{exploratory} filtering --- five linked sections,
  broad to narrow:
  \vspace{6pt}
  \begin{columns}[T]
    \column{0.19\textwidth}\centering
      {\color{gold}\textbf{1}}\\ \footnotesize Opening Lens\\ \scriptsize\color{slate} global scale \& geography
    \column{0.19\textwidth}\centering
    ...
    \column{0.19\textwidth}\centering
      {\color{gold}\textbf{4}}\\ \footnotesize Athlete Lens\\ \scriptsize\color{slate} age, gender, load
    \column{0.19\textwidth}\centering
      {\color{gold}\textbf{5}}\\ \footnotesize Paris Geography\\ \scriptsize\color{slate} venues \& records
  \end{columns}
  \vspace{8pt}
...
\begin{frame}{Which technique each chart applies}
  ...
  \begin{tabularx}{\textwidth}{@{}l X@{}}
    ...
    Medal-flow Sankey       & Multipartite graph; edge width = flow \\
...
\begin{frame}{Opening Lens --- global medal geography}
  \begin{columns}[c]
    ...
    \column{0.36\textwidth}\small
      Colour intensity makes the USA--China lead \teallead{pop pre-attentively}
      before any reading. Olympic NOC codes are mapped to ISO-3 so geometry
      resolves.\\[6pt]
...
\begin{frame}{A derived flow graph}
  \begin{columns}[c]
    ...
    \column{0.34\textwidth}\small
      NOC~$\rightarrow$~discipline~$\rightarrow$~medal colour as three node layers;
      \teallead{ribbon width = record volume}. Honest about being a categorical
      flow, not a social network.\\[6pt]
...
% --------------------------- VIZ 6: VENUE MAP -----------------------
\begin{frame}{Paris Geography --- the spatial footprint}
  \begin{columns}[c]
    \column{0.62\textwidth}\centering
      \includegraphics[width=\linewidth]{figures/venue_geography.pdf}
    \column{0.36\textwidth}\small
      A \teallead{symbol map}: bubble size = athlete volume, colour = medal
      activity. The dense Paris cluster contrasts with sailing/surf venues far
      to the south.\\[6pt]
...
\begin{frame}{Interaction and storytelling}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      \teallead{Interaction}
      \begin{itemize}
        \item \textbf{No sidebar} --- tabs switch sections; controls sit beside each chart
    ...
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        \item Author-driven arc, reader-driven on each page
        \item Each section leads with \textbf{headline KPI cards}, then answers visually
  ...
  \begin{block}{Where we drew the line}
    \footnotesize \textbf{Animation \emph{is} used} --- but only on the
    time-ordered medal race, where motion carries information. Uncertainty bands
    and trend smoothing are left out: a completed-event snapshot has no
    sampling error to show.
  \end{block}
\end{frame}
...
\begin{frame}{Key insights}
  ...
  \begin{enumerate}
    \item \teallead{Gold was a dead heat, totals were not.} USA \& China each 40
      gold; USA led totals 126--91 through breadth.
...
% --------------------------- CLOSE ----------------------------------
\begin{frame}{Reproducible by design}
  \small
  \begin{columns}[T]
    \column{0.55\textwidth}
      \teallead{Figures match the dashboard}
      \begin{itemize}
        \item Every figure is generated from the same code that drives the live dashboard
        \item No screenshots and no hand-drawn charts --- the report cannot drift from the app
        \item A single pipeline regenerates all figures from the source data
      \end{itemize}
    \column{0.45\textwidth}
      \teallead{Checked}
      \begin{itemize}
        \item All five sections and every control exercised, including empty and narrow selections
        \item Empty selections degrade gracefully to a styled ``no data'' state
        \item Row counts, data types, and missing-data rates reconcile with the source
      \end{itemize}
  \end{columns}
  \vfill
  \begin{center}
    {\color{gold}\rule{0.7\paperwidth}{1.6pt}}\\[6pt]
    {\large\bfseries\color{charcoal} Thank you} \quad
    {\color{slate} Questions welcome.}
  \end{center}
\end{frame}
